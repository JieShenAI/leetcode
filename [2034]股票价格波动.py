...
#  
#  Related Topics 设计 哈希表 数据流 有序集合 堆（优先队列） 👍 23 👎 0
...
class StockPrice:
    ...
    def __init__(self):
        self.stock_price = SortedList()
        self.times = {}
        self.cur = -1

    def update(self, timestamp: int, price: int) -> None:
        if timestamp not in self.times:
            self.times.setdefault(timestamp, price)
        else:
            old = self.times[timestamp]
            self.stock_price.remove(old)
            self.times[timestamp] = price
        self.stock_price.add(price)
        self.cur = max(self.cur, timestamp)

    def current(self) -> int:
        return self.times[self.cur]

    def maximum(self) -> int:
        return self.stock_price[-1]
